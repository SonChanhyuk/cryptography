...
{
    uint32_t s[Nk];
    memcpy(s,state,Nb*Nk);

	for (int i=0;i<Nb;i++){
        *(s+i) ^= *(roundKey+i);
	}
    memcpy(state,s,Nb*Nk);
}

static void SubBytes(uint8_t *state, int mode)
{
    const uint8_t *box = (mode ? sbox : isbox);
...
static void ShiftRows(uint8_t *state, int mode)
{
    ...
    if(mode == ENCRYPT){
        for (int r=0;r<Nb;r++){
            for (int c=0;c<Nk;c++){
                tmp[c*Nb+r] = *(state+((c+r)*Nb+r)%(Nb*Nk));
            }
        }
        memcpy(state,tmp,Nb*Nk);
    } else {
        for (int r=0;r<Nb;r++){
            for (int c=0;c<Nk;c++){
                tmp[c*Nb+r] = *(state+((c+4-r)*Nb+r)%(Nb*Nk));
            }
        }
        memcpy(state,tmp,Nb*Nk);
    }
...
{
    const uint8_t *metrix = (mode ? M : IM);
    uint8_t s[Nk*Nb];
    memcpy(s,state,Nk*Nb);
    uint8_t r,a,b;
    ...
    for (int col=0;col<Nk;col++){
        for (int row=0;row<Nb;row++){
            ...
            //s의 col과 M의 row를 gf(2^8) mod(x^8+x^4+x^3+x+1)에서 각각 곱한 다음 XOR연산으로 합칩니다
            //r에는 i가 0~3까지의 값을 모두 합한다음 최종적으로 *(state+col*Nb+row)에 저장합니다.
            a = s[col*Nb+1];
            b = metrix[row*Nb+1];
            while (b > 0){
                if (b & 1) r ^= a;
...
void KeyExpansion(const uint8_t *key, uint32_t *roundKey)
{
    ...
    for (int i=Nk;i<Nb*(Nr+1);i++){
        ...
        if (i % Nk == 0){
            ...
            p[3] = sbox[q];
            p[0] ^= Rcon[i/Nk];
        } else if (Nk > 6 && i % Nk == 4){ //256bits(Nk == 8)인경우
            p = (uint8_t *)&tmp;
            p[0] = sbox[p[0]];
            p[1] = sbox[p[1]];
            p[2] = sbox[p[2]];
